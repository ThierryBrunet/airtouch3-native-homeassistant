const DAIKIN_PANEL_VERSION = "v22";
...
class DaikinAcPanel extends HTMLElement {
  ...
  _smoothTempHistory(points) {
    ...
    for (let i = 1; i < quantized.length; i += 1) {
      if (quantized[i].value !== out[out.length - 1].value) {
  ...
  _strokeTempLine(ctx, points, xAt, yAt, start, end) {
    if (!points?.length) {
      return;
    }
    const path = [
      { time: start, value: points[0].value },
      ...points,
      { time: end, value: points[points.length - 1].value },
    ];
    ctx.lineCap = "round";
    ctx.lineJoin = "round";
    ctx.beginPath();
