const DAIKIN_PANEL_VERSION = "v20";
...
class DaikinAcPanel extends HTMLElement {
  ...
  _bucketPointsByPixel(points, xAt, plotLeft, plotWidth) {
    if (!points?.length || plotWidth <= 0) {
      return [];
    }
    const buckets = Array.from({ length: plotWidth }, () => []);
    for (const point of points) {
      const col = Math.min(
        plotWidth - 1,
        Math.max(0, Math.floor(xAt(point.time) - plotLeft))
      );
      buckets[col].push(point);
    }
    const out = [];
    for (let col = 0; col < plotWidth; col += 1) {
      const bucket = buckets[col];
      if (!bucket.length) {
        continue;
      }
      const medianValue = this._median(bucket.map((point) => point.value));
      const latest = bucket.reduce((best, point) =>
        point.time >= best.time ? point : best
      );
      out.push({
        time: latest.time,
        value: this._quantizeTempValue(medianValue),
      });
    }
    return out;
  }

  _prepareTempRenderPoints(points, xAt, plotLeft, plotWidth) {
    const quantized = (points || []).map((point) => ({
      time: point.time,
      value: this._quantizeTempValue(point.value),
    }));
    const smoothed = this._compressPlateauPoints(quantized, 0.5);
    const bucketed = this._bucketPointsByPixel(
      smoothed,
      xAt,
      plotLeft,
      plotWidth
    );
    return this._compressPlateauPoints(bucketed, 0.5);
  }
  ...
  _renderLineChart(canvas, legend, options) {
    ...
    for (const line of series) {
      ...
      const renderPoints = stepLines
        ? this._prepareDamperRenderPoints(line.points)
        : this._prepareTempRenderPoints(
            line.points,
            xAt,
            pad.left,
            plotW
