const DAIKIN_PANEL_VERSION = "v21";
...
class DaikinAcPanel extends HTMLElement {
  ...
  _tempPlateauSegments(points, start, end) {
    if (!points?.length) {
      return [];
    }
    const segments = [];
    for (let i = 0; i < points.length; i += 1) {
      segments.push({
        start: i === 0 ? start : points[i].time,
        end: i < points.length - 1 ? points[i + 1].time : end,
  ...
  _strokeStepLine(ctx, points, xAt, yAt, start, end) {
    const segments = this._tempPlateauSegments(points, start, end);
    if (!segments.length) {
      return;
    }
    ctx.lineCap = "butt";
    ctx.lineJoin = "miter";
    ctx.beginPath();
    let y = yAt(segments[0].value);
    ctx.moveTo(xAt(start), y);
    for (const segment of segments) {
      const xStart = xAt(segment.start);
      const xEnd = xAt(segment.end);
      const nextY = yAt(segment.value);
      if (nextY !== y) {
        ctx.lineTo(xStart, y);
        ctx.lineTo(xStart, nextY);
        y = nextY;
      }
      ctx.lineTo(xEnd, y);
    }
  ...
  _strokeSmoothLine(ctx, points, xAt, yAt, start, end, stepLines) {
    if (stepLines) {
      this._strokeStepLine(ctx, points, xAt, yAt, start, end);
      return;
    }
    ctx.lineCap = "butt";
    ctx.lineJoin = "miter";
