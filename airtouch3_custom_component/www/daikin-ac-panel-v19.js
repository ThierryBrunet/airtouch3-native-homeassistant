const DAIKIN_PANEL_VERSION = "v19";
...
class DaikinAcPanel extends HTMLElement {
  ...
  _bucketPointsByPixel(points, xAt, plotLeft, plotWidth) {
    ...
    const buckets = new Array(plotWidth);
    for (const point of points) {
      const col = Math.min(
        plotWidth - 1,
        Math.max(0, Math.floor(xAt(point.time) - plotLeft))
      );
      const existing = buckets[col];
  ...
  _prepareTempRenderPoints(points, xAt, plotLeft, plotWidth) {
    ...
    return this._compressPlateauPoints(bucketed, 0.25);
  }
