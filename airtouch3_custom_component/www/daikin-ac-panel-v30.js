const DAIKIN_PANEL_VERSION = "v30";
...
class DaikinAcPanel extends HTMLElement {
  ...
  setConfig(config) {
    ...
    this.config = {
      ...
      outside_temperature_sensor:
        config.outside_temperature_sensor ||
        "sensor.aqara_motion4_device_temperature",
    };
    this._pending = {
      power: {},
      target: {},
  ...
  _hasPendingUI() {
    ...
    return (
      ...
      Object.keys(p.damperControlMode).length > 0
    );
  }
  ...
  _clearPendingIfConfirmed(hass, climate) {
    ...
    for (const zone of this.config.zones) {
      ...
      const pendingMode = this._pending.damperControlMode[zone.switch];
      if (pendingMode && pendingMode !== "ha") {
        const actualMode = this._protocolDamperControlMode(
          zone.switch ? hass.states[zone.switch] : undefined
        );
        if (pendingMode === actualMode) {
          delete this._pending.damperControlMode[zone.switch];
        }
      }
    }
  }

  _protocolDamperControlMode(switchState) {
    const type = switchState?.attributes?.zone_temperature_type;
    if (type === 0) {
      return "manual";
    }
    const mode = switchState?.attributes?.damper_control_mode;
    return mode === "manual" ? "manual" : "auto";
  }

  _zoneDamperControlMode(hass, zone) {
    const pending = this._pending.damperControlMode?.[zone.switch];
    if (pending) {
      return pending;
    }
    try {
      if (localStorage.getItem(`daikin-damper-mode:${zone.switch}`) === "ha") {
        return "ha";
      }
    } catch (_error) {
    ...
    const sw = zone.switch ? hass.states[zone.switch] : undefined;
    return this._protocolDamperControlMode(sw);
  }
  ...
  _climateView(hass) {
    ...
    const acUnitTemp = attrs.room_temperature ?? current;
    const temperatureLine =
      tempSource === "daikin_ac"
        ? `${acUnitTemp}°C (AC unit · return air)`
  ...
  _renderShell() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .zone.on .damper-mode-btn:not(.active) {
          border-color: #64b5f6;
          background: rgba(0, 0, 0, 0.2);
          color: #e3f2fd;
        }
        .damper-mode-btn.active,
        .zone.on .damper-mode-btn.active {
          background: #66bb6a;
          border-color: #81c784;
          color: #1a1a1a;
        }
        .damper-mode-btn.ha-reserved:not(.active) {
          opacity: 0.85;
        }
        .zone-damper-controls.hidden {
          display: none;
        }
      ...
      <ha-card>
        <div class="panel">
          ...
          <div class="chart-section">
            <div class="chart-title">Zone temperatures · 7 days (green band = unit ON; dotted = AC return-air; dashed = Alfresco outside)</div>
  ...
  _renderZoneShell(zone, index) {
    return `
      <div class="zone off" data-zone-index="${index}">
        ...
        <div class="zone-control">
          <div class="zone-damper-controls hidden" data-field="damper-controls">
            <button
              class="zone-control-btn"
          ...
          <div class="zone-damper-controls hidden" data-field="damper-controls-up">
            <button
              class="zone-control-btn"
        ...
        <div class="damper-mode-bar" data-field="damper-mode-bar">
          <button class="damper-mode-btn" data-action="damper-mode" data-mode="auto" data-switch="${zone.switch}">Auto</button>
          <button class="damper-mode-btn" data-action="damper-mode" data-mode="manual" data-switch="${zone.switch}">Manual</button>
          <button class="damper-mode-btn ha-reserved" data-action="damper-mode" data-mode="ha" data-switch="${zone.switch}">HA</button>
        </div>
      </div>`;
  }
  ...
  _updateZone(hass, zone, index) {
    ...
    const damperMode = this._zoneDamperControlMode(hass, zone);
    const canSetDamper =
      damperValue !== null && damperMode === "manual";
    const showDamperControls = damperMode === "manual";
    ...
    root.querySelectorAll('[data-field="damper-controls"], [data-field="damper-controls-up"]').forEach((el) => {
      el.classList.toggle("hidden", !showDamperControls);
    });
    root.querySelectorAll('[data-action="damper-mode"]').forEach((btn) => {
      btn.classList.toggle("active", btn.dataset.mode === damperMode);
    });
  }
  ...
  async _loadTempChartHistory(period) {
    ...
    const climateEntity = this.config.climate;
    ...
    this._chartSeries = sensorEntities.map((entityId, index) => ({
      ...
      points: this._smoothTempHistory(
        this._historyPoints(tempHistory[entityId] || [], true)
      ),
    }));

    let acUnitRows = [];
    if (climateEntity && this._hass?.states[climateEntity]) {
      try {
        const climateHistory = await this._hass.callWS({
          type: "history/history_during_period",
          ...period,
          entity_ids: [climateEntity],
          minimal_response: false,
          no_attributes: false,
          significant_changes_only: true,
        });
        const climateMap = this._normalizeHistoryMap(climateHistory, [
          climateEntity,
        ]);
        acUnitRows = climateMap[climateEntity] || [];
      } catch (error) {
        console.warn("Daikin AC unit temperature history load failed", error);
      }
    }

    let acUnitPoints = this._smoothTempHistory(
      this._historyAcUnitTempPoints(acUnitRows)
    );
    if (!acUnitPoints.length && climateEntity) {
      const climate = this._hass.states[climateEntity];
      const attrs = climate?.attributes || {};
      const raw = attrs.room_temperature ?? attrs.current_temperature;
      const value = Number(raw);
      if (Number.isFinite(value)) {
        acUnitPoints = [{ time: Date.now(), value }];
      }
    }
    if (acUnitPoints.length) {
      this._chartSeries.push({
        entityId: climateEntity,
        name: "AC unit",
        color: "#e0e0e0",
        dotted: true,
        points: acUnitPoints,
    ...
    const outsideEntity = this.config.outside_temperature_sensor;
    if (outsideEntity && this._hass?.states[outsideEntity]) {
      try {
        const outsideHistory = await this._hass.callWS({
          type: "history/history_during_period",
          ...period,
          entity_ids: [outsideEntity],
          minimal_response: true,
          no_attributes: true,
          significant_changes_only: true,
        });
        const outsideMap = this._normalizeHistoryMap(outsideHistory, [
          outsideEntity,
        ]);
        const outsidePoints = this._smoothTempHistory(
          this._historyPoints(outsideMap[outsideEntity] || [], true)
        );
        if (outsidePoints.length) {
          this._chartSeries.push({
            entityId: outsideEntity,
            name: "Alfresco outside",
            color: "#4fc3f7",
            dashed: true,
        ...
        console.warn("Daikin outside temperature history load failed", error);
      }
    }
  ...
  _historyAcUnitTempPoints(rows) {
    ...
    for (const row of rows || []) {
      ...
      let raw = attrs.room_temperature ?? attrs.current_temperature;
  ...
  _quantizeTempValue(value) {
    return Math.round(value * 4) / 4;
  }

  _valuesNear(a, b, epsilon = 0) {
    return epsilon > 0 ? Math.abs(a - b) < epsilon : a === b;
  }

  _compressPlateauPoints(points, epsilon = 0) {
    if (!points?.length) {
      return [];
    }
    const out = [points[0]];
    for (let i = 1; i < points.length; i += 1) {
      ...
      if (this._valuesNear(cur.value, prev.value, epsilon)) {
  ...
  _smoothTempHistory(points) {
    const minGapMs = 10 * 60 * 1000;
    const quantized = (points || []).map((point) => ({
      time: point.time,
      value: this._quantizeTempValue(point.value),
    }));
    if (!quantized.length) {
      return [];
    }
    const out = [quantized[0]];
    for (let i = 1; i < quantized.length; i += 1) {
      ...
      if (cur.time - prev.time < minGapMs) {
        out[out.length - 1] = cur;
      } else {
        out.push(cur);
      }
  ...
  _prepareDamperRenderPoints(points) {
    const rounded = (points || []).map((point) => ({
      time: point.time,
      value: Math.round(point.value),
    }));
    return this._compressPlateauPoints(rounded, 0);
  }

  _strokeTempLine(ctx, points, xAt, yAt, _start, end) {
    if (!points?.length) {
      return;
    }
    ...
    for (let i = 1; i < points.length; i += 1) {
      ctx.lineTo(xAt(points[i].time), yAt(points[i].value));
    }
    if (last.time < end) {
      ctx.lineTo(xAt(end), yAt(last.value));
    }
    ctx.stroke();
  }

  _strokeDamperStepLine(ctx, points, xAt, yAt, start, end) {
    const plateaus = this._prepareDamperRenderPoints(points);
    if (!plateaus.length) {
      return;
    }
    ...
    for (let i = 0; i < plateaus.length; i += 1) {
      ...
      const segEnd = i < plateaus.length - 1 ? plateaus[i + 1].time : end;
  ...
  _strokeSmoothLine(ctx, points, xAt, yAt, start, end, stepLines) {
    if (stepLines) {
      this._strokeDamperStepLine(ctx, points, xAt, yAt, start, end);
      return;
    }
    this._strokeTempLine(ctx, points, xAt, yAt, start, end);
  }
  ...
  _renderLineChart(canvas, legend, options) {
    ...
    for (const line of series) {
      ...
      const renderPoints = line.points;
      if (!renderPoints.length) {
        continue;
      }
      ctx.strokeStyle = line.color;
      ctx.lineWidth = line.dashed || line.dotted ? 1.5 : 1;
      ctx.setLineDash(
        line.dotted ? [2, 4] : line.dashed ? [6, 4] : []
      );
  ...
  renderZone(hass, zone) {
    ...
    return `
      <div class="zone ${isOn ? "on" : "off"}">
        ...
        <div class="zone-control">
          <div class="zone-damper-controls ${showDamperControls ? "" : "hidden"}">
            <button
              class="zone-control-btn"
              data-action="zone-damper-down"
              data-fan="${zone.fan}"
        ...
        <div class="damper-mode-bar">
          <button class="damper-mode-btn ${damperMode === "auto" ? "active" : ""}" data-action="damper-mode" data-mode="auto" data-switch="${zone.switch}">Auto</button>
          <button class="damper-mode-btn ${damperMode === "manual" ? "active" : ""}" data-action="damper-mode" data-mode="manual" data-switch="${zone.switch}">Manual</button>
          <button class="damper-mode-btn ha-reserved ${damperMode === "ha" ? "active" : ""}" data-action="damper-mode" data-mode="ha" data-switch="${zone.switch}">HA</button>
        </div>
      </div>`;
  }
  ...
  bindEvents() {
    ...
    root.querySelectorAll("[data-action='damper-mode']").forEach((btn) => {
      btn.onclick = (event) => {
        event.stopPropagation();
        this._setZoneDamperControlMode(btn.dataset.switch, btn.dataset.mode);
      };
    });
  ...
  _setZoneDamperControlMode(switchEntity, mode) {
    if (!["auto", "manual", "ha"].includes(mode)) {
      return;
    }
    this._pending.damperControlMode[switchEntity] = mode;
    try {
      if (mode === "ha") {
        localStorage.setItem(`daikin-damper-mode:${switchEntity}`, "ha");
      } else {
        localStorage.removeItem(`daikin-damper-mode:${switchEntity}`);
      }
    } catch (_error) {
      /* ignore storage errors */
    }
    ...
    this._syncViewSnapshot();
    if (mode !== "ha") {
      this._hass.callService("airtouch3", "set_zone_damper_control", {
        entity_id: switchEntity,
        mode,
      });
    }
  ...
  _setZoneDamper(fanEntity, currentDamper, delta) {
    const zone = (this.config.zones || []).find((item) => item.fan === fanEntity);
    if (zone && this._zoneDamperControlMode(this._hass, zone) !== "manual") {
      return;
    }
