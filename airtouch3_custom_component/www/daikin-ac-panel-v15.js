class DaikinAcPanel extends HTMLElement {
  ...
  disconnectedCallback() {
    ...
    if (this._chartResizeDebounce) {
      window.clearTimeout(this._chartResizeDebounce);
      this._chartResizeDebounce = 0;
    }
  ...
  _setupChartResizeObserver() {
    ...
    this._chartResizeObserver = new ResizeObserver(() => {
      if (this._chartPainting) {
        return;
      }
      if (this._chartResizeDebounce) {
        window.clearTimeout(this._chartResizeDebounce);
      }
      this._chartResizeDebounce = window.setTimeout(() => {
        this._chartResizeDebounce = 0;
        this._scheduleChartPaint();
      }, 300);
    });
    this._chartResizeObserver.observe(panel);
  }
  ...
  async _fetchDamperHistoryRows(entityId, switchId, period) {
    const entityIds = [entityId, switchId].filter(Boolean);
    for (const id of entityIds) {
      try {
        ...
        const rows = map[id] || [];
        const points = this._historyDamperPoints(rows, id.startsWith("switch."));
        if (points.length) {
          return points;
        }
      } catch (error) {
        console.warn(`Daikin damper history failed for ${id}`, error);
      }
    }
    return [];
  }
  ...
  _historyDamperPoints(rows, fromSwitch = false) {
    ...
    for (const row of rows || []) {
      ...
      if (points.length && points[points.length - 1].value === value) {
  ...
  _renderLineChart(canvas, legend, options) {
    ...
    for (const line of series) {
      ...
      ctx.lineWidth = 2;
      ctx.beginPath();
      if (stepLines && line.points.length) {
