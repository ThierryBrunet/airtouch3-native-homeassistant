class DaikinAcPanel extends HTMLElement {
  ...
  setConfig(config) {
    ...
    this._powerEntity = "binary_sensor.brunet_ac_power";
  }
  ...
  _chartEntities() {
    const sensors = this.config.zones
      .map((zone) => zone.sensor)
      .filter(Boolean);
    return [...sensors, this._powerEntity];
  }

  _ensureChart() {
    const canvas = this.shadowRoot?.querySelector("#temp-chart");
    if (!canvas || this._chartLoading) {
      return;
    }
  ...
  async _loadChartHistory() {
    ...
    try {
      ...
      const sensorEntities = entities.slice(0, -1);
      this._chartSeries = sensorEntities.map((entityId, index) => ({
        ...
        points: this._historyPoints(history[index] || [], true),
      }));
      this._powerSeries = this._historyPoints(
      ...
      console.warn("Daikin chart history load failed", error);
    } finally {
      this._chartLoading = false;
    }
  }
  ...
  _historyPoints(rows, numeric) {
    return (rows || [])
      .map((row) => {
        const time = new Date(row.lu || row.last_changed).getTime();
  ...
  _drawChart() {
    ...
    const yAt = (value) =>
      pad.top + plotH - ((value - minTemp) / (maxTemp - minTemp)) * plotH;
    ...
    if (this._powerSeries.length) {
      ...
      for (let i = 0; i < this._powerSeries.length; i += 1) {
        ...
        const nextTime =
          i < this._powerSeries.length - 1
            ? this._powerSeries[i + 1].time
            : end;
        if (point.value === 1) {
    ...
    if (legend) {
      const items = this._chartSeries
        .map(
          (series) =>
            `<span class="chart-legend-item"><span class="chart-legend-swatch" style="background:${series.color}"></span>${series.name}</span>`
        )
        .join("");
      legend.innerHTML = `${items}<span class="chart-legend-item"><span class="chart-legend-swatch chart-legend-power"></span>Unit ON</span>`;
    }
  }
